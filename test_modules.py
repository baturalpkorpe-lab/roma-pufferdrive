...
test_modules.py
===============
Unit tests for ROMA (flat encoder) on PufferDrive.
Run this before every training run to verify all components are healthy.

Usage:
    cd ~/PufferDrive
    PYTHONPATH=.:roma_pufferdrive python3 -m unittest test_modules

If OBS_DIM tests fail, run check_env.py and update OBS_DIM below
...
EMB_DIM = 64    # flat encoder env_embed_dim
...
class TestRoleEncoder(unittest.TestCase):
    def setUp(self):
        ...
        self.hidden   = 64
        self.B        = 16
        self.enc      = RoleEncoder(EMB_DIM, self.role_dim, self.hidden)
    ...
    def test_forward_shapes(self):
        emb = torch.randn(self.B, EMB_DIM)
        h   = self._zero_hidden()
        self.enc.train()
        role_z, mu, logvar, new_h = self.enc(emb, h)
...
class TestAuxLoss(unittest.TestCase):
    def setUp(self):
        ...
        self.W        = 8
        self.fn       = RomaAuxLoss(
            role_dim=self.role_dim, emb_dim=EMB_DIM,
    ...
    def test_forward_keys(self):
        ...
        emb_win   = torch.randn(self.B, self.W, EMB_DIM)
        self.fn.train()
    ...
    def test_diversity_single_agent(self):
        out = self.fn(
            ...
            torch.randn(1, self.W, EMB_DIM),
        )
        self.assertAlmostEqual(out["div_loss"].item(), 0.0, places=5)


class TestRomaPolicyFlat(unittest.TestCase):
    def setUp(self):
        from roma.policy_flat import RomaPolicyFlat
        self.action_dim = 91
        self.role_dim   = 8
        self.B          = 4
        self.policy     = RomaPolicyFlat(
            obs_dim    = OBS_DIM,
    ...
    def test_emb_window_shift(self):
        ...
        with torch.no_grad():
            ...
            env_emb = self.policy.flat_enc(obs)
        self.assertTrue(
...
if __name__ == "__main__":
    print(f"Running ROMA flat-encoder module tests (OBS_DIM={OBS_DIM}) ...\n")
