...
train_roma_flat.py
==================
Train the ROMA policy with a FLAT MLP observation encoder on PufferDrive.

This script is identical to train_roma.py except it uses RomaPolicyFlat
from roma/policy_flat.py — a single MLP that compresses the full 1120-dim
observation into a 64-dim embedding, instead of the structured attention
encoders (EgoEncoder + PartnerEncoder + RoadEncoder).

Purpose: compare flat encoder vs structured attention encoder.

CPU TESTING (quick sanity check):
    PYTHONPATH=/root/PufferDrive/roma_pufferdrive python3 roma_pufferdrive/train_roma_flat.py \
        --role_dim 1 --num_maps 100 --total_steps 2000000 \
        --num_agents 16 --device cpu --save_interval 1000000 \
        --save_dir roma_pufferdrive/checkpoints/roma_flat_dim1_test
...
def init_wandb(args):
    if not args.wandb_project:
        print("[ROMA-FLAT] wandb disabled (no --wandb_project provided). Using CSV only.")
        return None
    ...
    try:
        ...
        if args.wandb_offline:
            ...
            print("[ROMA-FLAT] wandb running in offline mode.")

        run = wandb.init(
            project = args.wandb_project,
            entity  = args.wandb_entity or None,
            name    = f"roma_flat_dim{args.role_dim}_maps{args.num_maps}_{args.seed}",
            config  = vars(args),
            tags    = [f"flat_encoder", f"role_dim_{args.role_dim}", f"maps_{args.num_maps}"],
        )
        print(f"[ROMA-FLAT] wandb initialized: {run.url}")
        return run

    except ImportError:
        print("[ROMA-FLAT] wandb not installed. Falling back to CSV logging only.")
        return None
    except Exception as e:
        print(f"[ROMA-FLAT] wandb init failed: {e}. Falling back to CSV logging only.")
        return None
...
def parse_args():
    ...
    p.add_argument("--embed_dim",     type=int,   default=64,
                   help="Output size of the flat MLP encoder (obs → embed_dim).")
    ...
    p.add_argument("--num_minibatch", type=int,   default=12,
                   help="Minibatches per PPO epoch. 12 keeps the minibatch at "
                        "~65k samples with 3072 agents (matches the main "
                        "branch for a fair encoder comparison).")
    ...
    # Logging / saving
    p.add_argument("--save_dir",      type=str,   default="roma_pufferdrive/checkpoints/roma_flat")
...
def run_evaluation(args, policy, device, wandb_run=None):
    print("\n" + "=" * 60)
    print("  POST-TRAINING EVALUATION (FLAT ENCODER)")
    print("=" * 60)
...
def run_wosac_eval(args, policy, device, wandb_run=None, global_step=None,
    ...
    try:
        ...
        wosac_config = {
            "eval": {"wosac_init_steps": 0, "wosac_num_rollouts": rollouts},
...
def train(args):
    ...
    print(f"[ROMA-FLAT] role_dim      : {args.role_dim}")
    print(f"[ROMA-FLAT] num_maps      : {args.num_maps}")
    print(f"[ROMA-FLAT] num_agents    : {args.num_agents}")
    print(f"[ROMA-FLAT] total_steps   : {args.total_steps:,}")
    print(f"[ROMA-FLAT] mi_weight     : {args.mi_weight}")
    print(f"[ROMA-FLAT] div_weight    : {args.div_weight}")
    ...
    action_dim   = 91
    print(f"[ROMA-FLAT] obs_dim       : {obs_dim} (auto-detected)")

    policy = RomaPolicyFlat(
        obs_dim        = obs_dim,
    ...
    print(f"[ROMA-FLAT] CSV log       : {log_csv}")
    ...
    while global_step < args.total_steps:
                    ...
                    for item in info:
                        if isinstance(item, dict):
                            ...
                            if "episode_return" in item:
                                ep_returns.append(float(np.mean(item["episode_return"])))
        ...
        if global_step % args.log_interval < B * args.rollout_steps:
            ...
            print(f"[ROMA-FLAT step {global_step:>12,}] "
                  f"sps={sps:>7,.0f}  "
        ...
        if global_step >= next_save:
            ckpt = Path(args.save_dir) / \
                   f"roma_flat_dim{args.role_dim}_step{global_step}.pt"
            torch.save({
            ...
            print(f"[ROMA-FLAT] Saved -> {ckpt}")
            next_save += args.save_interval

        # ---- Periodic WOSAC evaluation (lite) ----
        if global_step >= next_wosac:
            print(f"[ROMA-FLAT] Periodic WOSAC eval at step {global_step:,} ...")
            run_wosac_eval(
    ...
    # ---- Final save ----
    final = Path(args.save_dir) / f"roma_flat_dim{args.role_dim}_final.pt"
    torch.save({
        "global_step" : global_step,
        "policy_state": policy.state_dict(),
        "args"        : vars(args),
    }, final)
    print(f"[ROMA-FLAT] Training complete. Saved -> {final}")
