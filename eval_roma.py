...
def run_role_analysis(policy, env, num_episodes, role_dim, device,
    ...
    pre  = np.cumsum(tele, axis=1) == 0                # before the first jump
    n_pre = pre.sum(axis=1)
    seg_max = np.where(pre, spd, -np.inf).max(axis=1)
    seg_max = np.where(n_pre >= 10, seg_max, np.nan).reshape(-1)
