...
for v in ROLE_ROAD_DIM ROLE_PARTNER_DIM MI_TARGET MI_WEIGHT SEED SCRATCH_ROOT \
         ...
         COMPLIANCE_PERTURBED_ONLY MI_EXCLUDE_PERTURBED PUFFER_DIR TOTAL_STEPS WANDB_PROJECT WANDB_ENTITY WANDB_NAME \
         INIT_FROM FREEZE_ROLE_ENCODER SAVE_INTERVAL \n         COLLISION_BEHAVIOR OFFROAD_BEHAVIOR GOAL_SPEED \n         REWARD_VEHICLE_COLLISION REWARD_OFFROAD_COLLISION; do
    eval "val=\${$v:-}"
    [ -n "$val" ] && E="$E,$v=$val"
done
