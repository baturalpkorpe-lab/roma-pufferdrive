#!/bin/bash
# FUTURE-MI x AGENT-CENTRIC at ROLE_DIM=1, div loss OFF (div_weight=0). Launched by:
#
#   DIV_WEIGHT=0 bash slurm/run_mi_future.sh 1 8
#
# Pairs with analyze_mifutagent_dim1.sh (the div-on arm). BOTH arms are required: the
...
RUN_DIR=roma_mifutagent_dim1_H8_r16p64_ep_nodiv
TAG=mifutagent_dim1_nodiv
ROLE_DIM=1
SKIP_ICC=1
